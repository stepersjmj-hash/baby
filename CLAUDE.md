...
- 탭으로 세고 숫자를 고르는 **세어보기** (판정 엔진 없음 — 전부 click)
...
js/
  ...
  count/
    index.js              세어보기: 물건 탭하면 세어 주고, 숫자 카드로 답한다
    levels.js             문제 15개 (하·중·상 각 5, 씨앗 난수) — 점검과 공용
  spot/
...
```js
await eval(await (await fetch('/tools/selftest-count.js')).text())
```

15문제 구성 · 개수 범위 · 숫자 카드 · 방해꾼 규칙 · 겹침 없음을 확인한다.

다섯 다 전부 `OK` 여야 한다. 판정은 전부 엔진을 직접 불러 시험하므로
아이가 모은 별은 지워지지 않는다.
...
- **내 사진 밑그림**: 그림 고르기 시트의 📷 카드로 추가한다. `photopage.js` 가
  캐니(Canny) 방식으로 외곽선을 딴다: 채널별 대비 보정 → 미디언+블러 →
  소벨 → 비최대 억제 → 이력 문턱 → 질감 억제(엣지 밀도) → 조각 걸러내기
  (작으면서 약한 것만 버림 — 눈·입 같은 작고 강한 디테일은 살아남는다) →
  8방향 2회 굵히기. 투명 PNG 로 IndexedDB(linework) 저장, 카드 2초 길게 눌러 삭제.
  **흑백이 아니라 R·G·B 채널별로 기울기를 재는 게 핵심이다** — 살구색 얼굴 vs
  하늘처럼 밝기가 같고 색만 다른 경계는 흑백으론 안 잡혀서 물감통이 샌다.
  문턱은 백분위(p82)라 사진마다 적응한다.
...
`js/count/levels.js` 의 `LEVELS` 에 한 줄. 물건 배치·개수·숫자 카드는
씨앗 난수로 생성된다. 상 난이도는 `decoys` 에 방해꾼 이모지를 준다
("오리만" 세기 — 방해꾼은 탭해도 조용하다).

```js
L('c16', '바나나', '🍌', 2, 149)                      // 중: 6~10개
L('c17', '사탕만', '🍬', 3, 151, ['🍭', '🍫'])        // 상: 방해꾼 섞임
```

- **조사는 손으로 쓰지 말 것.** `ask` 의 이/가는 `josa()` 가 받침으로
  계산한다. 손으로 쓰면 "별가 몇 개일까?" 가 나오고, 음성 클립까지
  그대로 녹음돼 버린다 (실제로 그랬다). 이름만 넣으면 된다.
- **조작이 전부 탭(click)인 이유**: iOS 실측 규칙상 click 안에서만 소리
  권한이 확실하다. 그래서 세는 소리("하나, 둘…")·정답 읽기("세 개!")가
  탭 순간 바로 나온다. 판정 엔진도 펜 입력도 안 쓴다.
- 센 물건을 다시 탭하면 배지가 빠진다 — 다시 셀 수 있다.
- 추가한 뒤 `tools/selftest-count.js` 로 확인한다.
...
- **더 자연스러운 목소리로 교체**: TTS 사이트(Airy Studio 등)에서 생성한
  ...
  **지금은 144문구 전부 이 방식으로 교체돼 있다** — 유나 합성은 폴백일 뿐이다.
- **교체 파일은 언제나 다시 변환한다.** 예전엔 "wav 가 m4a 보다 최신일
  때만" 바꿨는데, 문구를 고쳐 유나로 새 클립을 만든 **직후에** 좋은 목소리
  wav 를 넣으면 m4a 가 더 최신이라 영영 안 바뀐다 ("넣었는데 왜 그대로지").
  afconvert 는 같은 입력에 같은 바이트를 내므로 매번 돌려도 git 은 깨끗하다.
- **문구를 고치면 옛 클립이 고아로 남는다** → 생성기가 매니페스트에 없는
  m4a 를 지운다. **비교는 반드시 NFC 로** — 맥이 파일명을 NFD 로 돌려줄 때가
  있어서, 그냥 비교하면 멀쩡히 쓰는 클립을 고아로 오인해 지운다 (겪었다).
...
- **킥보다 먼저 태어난 컨텍스트는 킥이 돌아도 무음일 수 있다.** 첫 획을
  긋는 동안(pointerdown) 만들어진 컨텍스트가 정확히 이 처지다. 킥의 첫
  'playing' 에서 **표시만 해 두고**(`needRebuild`), 다음 제스처의 `unlock()`
  에서 새로 판다. 그래서 `voice()` 의 move/stop 은 닫힌 컨텍스트에도
  안전해야 한다 (try/catch).
- **★ 컨텍스트를 *만드는* 것도 반드시 제스처 안이어야 한다.** 소리를 내는
  것만 제스처 안이면 된다고 생각하기 쉬운데 아니다 — 제스처 밖에서 만든
  AudioContext 를 iOS 는 suspended 로 붙잡아 두고 `resume()` 도 안 먹인다.
  그러면 효과음·클립이 전부 WebAudio 를 타므로 **소리가 통째로 죽는다.**
  한 번 이걸로 앱 전체가 무음이 됐다: 킥의 'playing' 핸들러(제스처 아님)
  안에서 `resetAudio()` 를 불렀던 것. **컨텍스트 재생성은 미뤘다가
  click·touchend·pointerup 안에서 한다.** `unlock()` 이 그 자리다.
- **굳은 컨텍스트는 되살리려 애쓰지 말고 버린다.** 지난 제스처에서
  `resume()` 을 부르고도 여태 running 이 아니면 새로 파는 게 빠르고 확실하다
  (`unlock()` 의 `wokeLast`). 어떤 경로로 죽든 탭 한 번이면 복구된다.
- **원인 추적은 `tools/ios-check.html`** — 아이패드에서 열면 자동 검사에
  버튼 5개(효과음/TTS/재생성/일반오디오/우회로)와 출력 미터가 있다.
  미터가 뛰는데 무음이면 세션·기기 쪽, 미터가 0 이면 코드 쪽.
  단 **ios-check 는 증상을 가린다** — 조작이 전부 버튼 click 이라 굳은
  컨텍스트가 매 탭마다 복구돼 버린다. "진단은 되는데 앱은 무음" 이면
  앱만 하는 짓(제스처 밖 컨텍스트 생성)을 의심할 것.
- **홈 화면이 스스로 알린다.** 아이패드에는 콘솔이 없어서, `main.js` 가
  `audioState()` 를 홈 아래에 찍는다 — **이상할 때만**(running 아님 ·
  샘플레이트 24000 · 킥 오류 · 음소거). 평소엔 빈 줄이라 아이 화면이
  깨끗하다. `탭:N` 은 제스처가 창까지 오는지 보는 계수기 —
  "안 눌렀다" 와 "눌렀는데 안 먹었다" 를 가른다.
...
- **`tools/selftest.js` 의 전체 지우기 단계는 백그라운드 탭에서 실패한다.** 길게 누르기(1초)가
