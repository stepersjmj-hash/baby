...
구현된 콘텐츠는 세 갈래다.
- **색칠하기**
- 궤적 판정 엔진(`core/trace.js`)을 공유하는 여섯:
  **따라 그리기 · 한글 쓰기 · 숫자 쓰기 · 영어 쓰기 · 미로 찾기 · 점 잇기**
- 끌어다 맞추는 **조각 퍼즐** (짝 맞추기를 교체 — 이전 코드는 git 히스토리에)
- 동그라미 판정(`spot/judge.js`)을 쓰는 **다른 그림 찾기**
...
js/
  ...
  trace/                  ← 여섯 활동이 화면·배선을 전부 공유한다
    ...
    dots.js               점 잇기 그림 5개
  puzzle/
    index.js              조각 퍼즐: 판·조각 그리기, 드래그, 착 붙기, UI 배선
    pics.js               그림 30장 (하·중·상 각 10) — 이모지 장면
    cut.js                조각 기하: 혹(knob) 달린 조각으로 자르기 — 점검과 공용
  spot/
    index.js              다른 그림 찾기: 두 그림판, 동그라미 표시, UI 배선
    scenes.js             문제 생성 — 주제 10 × 난이도 3 = 30문제 (씨앗 난수)
...
tools/selftest-trace.js   따라 그리기류 6코스 자가 점검 (진행 기록을 건드리지 않는다)
...
6코스 110단계 완주 · 길 밖 판정 · 질러가기 방지 · 획순을 확인한다.
...
30문제 구성(하·중·상 10개씩, 중복 없음) · 전부 찾아짐 · 문지르기 방지를 확인한다.
...
- `hard` 가 곧 조각 수다: 1→3조각(세로 3등분) · 2→5조각(띠+3칸+띠) · 3→8조각(4×2).
  자르는 모양을 바꾸려면 `cut.js` 의 `LAYOUTS` 를 고친다.
- 혹(knob)의 방향은 씨앗 난수로 정해진다. **씨앗은 P() 가 자동으로 붙이므로
  건드릴 필요 없다** — 같은 문제는 언제나 같은 모양으로 잘리고 같은 자리에서 시작한다.
- 조각은 **자기 자리 근처(SNAP=48)에서만** 붙는다. 조각 중심끼리 SNAP 보다
  가까우면 안 된다 — 자가 점검이 이걸 확인한다.
- 다시하기(↩️)만 무작위로 새로 흩는다. 첫 열기가 결정적이라 점검이 드래그를
  흉내낼 수 있다.
- 추가한 뒤 `tools/selftest-puzzle.js` 로 "자르기 OK / 혹홈 OK" 를 확인한다.

## 다른 그림 찾기 문제 추가하기

`js/spot/scenes.js` 만 고친다. 문제는 손으로 그리지 않는다 — **주제(이모지 풀)와
씨앗만 정의하면 생성기가 만든다.** 같은 씨앗이면 언제나 같은 문제다.

- **주제 추가**: `THEMES` 에 풀 14개 + 비슷한 짝(`subtle`) 4쌍을 넣으면
  난이도별로 문제가 3개 늘어난다 (`SPOTS` 는 주제 × 난이도로 자동 생성).
- **풀은 14개를 유지할 것.** 상 난이도가 12개를 쓰므로 바꿔치기용 여분이
  최소 2개 필요하다.
- `subtle` 짝(🐓→🐔)이 상 난이도의 핵심이다. 비슷하게 생긴 이모지로 고를 것.
  너무 최신 이모지는 구형 아이패드에서 □ 로 나오니 피한다.
- 난이도가 올리는 것: 물건 수 6→9→12 · 다른 곳 수 2→3→4 · 바꾸는 방식
  (전혀 다른 그림 → 섞어서 → 비슷한 그림 + 없어진 것).
- 판정은 한 획에 다른 곳 하나만 인정한다 — 화면 전체를 문질러도 소용없다.
- 추가한 뒤 `tools/selftest-spot.js` 로 "구성 OK / 전부 찾아짐 OK" 를 확인한다.
...
여섯 활동(선 긋기·한글·숫자·영어·미로·점 잇기)이 화면 하나와 러너 하나를 공유한다.
