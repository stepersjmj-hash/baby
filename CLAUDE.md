...
js/
  ...
  core/
    icons.js              SVG 아이콘 전부 — 홈 카드 · 트레이 · 색칠 도구 · 단계 칩
    pen.js                애플펜슬/손가락 입력 정규화, 손바닥 인식, 시드 난수
    fit.js                종이 크기·배율·원점 — 화면 다섯이 같이 쓴다
  ...
  trace/                  ← 일곱 활동이 화면·배선을 전부 공유한다
    ...
    numbers.js            숫자 획순 — 1~19 · 십 단위 20~100 (낱자를 좁혀 조립)
    english.js            알파벳 A~Z 획순 — 한 단계에 대문자+소문자 한 쌍
    names.js              이름 쓰기 — 자모 획을 음절 블록에 조판 (가족 이름 11개)
    maze.js               미로 14개 — 들어갈 때마다 새로 판다 (+ 최단 경로 탐색)
    dots.js               점 잇기 그림 10개 (쉬움 3 · 보통 2 · 어려움 5)
  puzzle/
    index.js              조각 퍼즐: 판·조각 그리기, 드래그, 착 붙기, 내 사진 관리
    pics.js               기본 그림 9장 (하·중·상 각 3) — 이모지 장면
    cut.js                조각 기하: BSP 무작위 분할 + 혹(knob) — 점검과 공용
  video/
    index.js              영상 보기 (숨김) — 설정에서 켜야 홈에 나온다.
                          목록·파일은 집 NAS 에 있다 (저장소엔 없다)
  count/
...
assets/voice/             음성 팩 (클립 153개 + manifest) — make-voice.mjs 로 생성
...
tools/nas-video-list.php  영상 폴더 목록 생성기 (NAS 의 video/list.php 로 둔다)
...
docs/획순.md              한글·영어·숫자 획순의 근거와 우리가 고른 방식
docs/stroke_order.json    획순 규칙 원본 (획수·방향·설명. 좌표는 없다)
design_handoff_home_redesign/
                          홈 화면 디자인 원본 (README + .dc.html 프로토타입 2개)
```

## 디자인

홈·서브 화면 디자인의 **기준 원본은 `design_handoff_home_redesign/README.md`** 다.
색·타이포·간격·아이콘 path 가 최종값으로 적혀 있으니, 화면을 고칠 일이 생기면
이 파일부터 읽고 그 값을 따를 것. `.dc.html` 은 레퍼런스 프로토타입이지
복사해 쓰는 코드가 아니다 — 이 저장소 방식(바닐라 JS + CSS 클래스)으로 재구현한다.

- 활동은 네 갈래(창작 · 쓰기 준비 · 생각 놀이 · 곧 나와요)로 묶인다.
  그룹 색은 `js/main.js` 의 `GROUPS` 한 곳에 있고, 라벨 점 · 아이콘 타일 배경 ·
  아이콘 선 · 카드 하단 띠에 함께 쓰인다. **글자를 못 읽는 나이라 색이 유일한 분류 단서다.**
- **홈에는 이모지를 쓰지 않는다.** 기기마다 그림이 다르고 색을 그룹에 맞출 수 없다.
  아이콘은 `js/main.js` 의 `ICON` 표에 48×48 인라인 SVG 로 있고, 그룹 stroke 색을
  인자로 받는다. 활동을 추가하면 `ACTIVITIES` 에 한 줄 + `ICON` 에 한 줄.
- **아이콘은 전부 `js/core/icons.js` 한 곳에 있다.** 네 갈래다:
  `ACT_ICON`(홈 카드 — 그룹 색을 인자로 받는 선 아이콘) · `icon(name)`(트레이·헤더
  선 아이콘) · `toolIcon(id)`(색칠 도구 10종 — 도구마다 제 색을 가진 **채움** 아이콘) ·
  `lvlIcon(코스, 단계id)`(단계 칩 — 선 긋기 22종은 **그 단계가 그릴 선**을 제 색
  선으로, 미로 14종은 목적지를 채움으로) · `picIcon(이름)`(콘텐츠 그림 25종 —
  점 잇기와 세어보기가 **칩과 스테이지에 같은 그림**을 쓴다).
  없는 키면 빈 문자열이라 러너가 `level.ico` 로 되돌아간다.
  여기에 스테이지 위 그림 `STAGE_ART` 가 더해진다.
  - **나선 계열(나선·달팽이집·큰 나선)만 굵기 2.6이다.** 감긴 간격이 4 안팎이라
    다른 아이콘처럼 4~5로 그리면 칩 크기(32px)에서 통째로 뭉개져 얼룩이 된다.
    핸드오프 프로토타입 값이 그랬는데, 코드에서 얇게 고쳤다.
  트레이 버튼은 `index.html` 의 `data-icon` 속성으로 표시만 해 두고 `main.js` 가
  그림을 채운다 — 홈 버튼 하나가 화면 5개에 있으므로 한 줄로 묶는 게 맞다.
- **선택·눌림 표시는 앱 전체가 한 규칙만 쓴다**: `--sel-bg`(#fff3df) + `--sel-br`(#ff8a3d).
  **움직이지 않는다** — 예전엔 도구가 위로 튀어올랐는데, 튀어오른 만큼 옆 도구와
  높이가 어긋나 어느 게 눌린 건지 오히려 헷갈렸다.
- **손잡이**(`localStorage['haichu.hand']`, 기본 `right`)를 `body[data-hand]` 로 걸고
  CSS 가 분기한다. **버튼은 쥐는 손 반대쪽**에 모으고, 남는 쪽은 `.palm`(손 쉼터)으로
  비운다 — 아이는 손바닥을 얹고 그리므로 얹을 자리를 만들어 주는 게
  palm rejection 만큼 중요하다. `.palm` 은 `pointer-events:none` 이다.
  왼손은 `row-reverse` 가 아니라 `.palm{order:-1}` 로 뒤집는다 — row-reverse 를 쓰면
  ◀ ▶ 순서까지 뒤집혀서 안 된다.
- 단계 칩의 난이도 띠는 레벨의 `hard`(1·2·3)로 켜진다. 따라 그리기류에서는
  **선 긋기와 점 잇기만** 난이도가 있다
  (선 긋기 1~8 쉬움 · 9~15 보통 · 16~22 어려움 / 점 잇기는 점 개수로 ~7 · 8~10 · 11~).
- 단계 칩은 `.lvl` 하나를 네 화면이 공유한다. 퍼즐 칩은 그 문제의 그림을 캔버스로
  줄여 넣는다(`drawScene`) — 아이콘보다 정보가 많고 내 사진 퍼즐도 그대로 된다.
- **세어보기·점 잇기는 이모지를 버리고 `picIcon` 으로 갔다.** 세어보기는 칩과
  스테이지가 **같은 그림**이어야 한다 — 아이가 칩을 보고 무슨 문제인지 알고
  스테이지에서 그 물건을 센다. 그래서 방해꾼까지 25종을 다 그렸다.
  `count/levels.js` 의 `ico`·`decoys` 는 이제 이모지가 아니라 **picIcon 키**다.
- 다른 그림 찾기 칩의 이모지는 아직 **콘텐츠**(그 장면)라 그대로 둔다.
- **짝 칩 `.lvl.pair`** 은 한 칩에 두 글자를 얹는다 (영어: 위 대문자 · 아래 소문자).
  좁은 화면에서 칩 라벨을 숨기는 규칙에서 빠진다 — 여기서 아래 글자는 이름표가
  아니라 배울 내용이다. 레벨에 `lbl` 을 주면 러너가 이 칩으로 그린다.
- 스테이지 위 그림은 코스 옵션 `art`(`{from,to}`)로 정한다 — `pen`·`star`·`runner` 는
  `icons.js` 의 `STAGE_ART`, `'level'` 이면 그 단계의 칩 아이콘을 크게 쓴다(미로 목적지).
  한글·숫자·영어·이름은 연필→별, 미로는 주인공→목적지다.
  **선 긋기의 캐릭터 쌍(🐝→🌻)만 이모지로 남겼다** — UI 가 아니라
  그 단계의 이야기라서다. 점 잇기는 도착이 곧 출발이라 그림을 안 그린다.
- **영상 보기는 숨김 기능이다.** `localStorage['haichu.video']` 가 `'on'` 일 때만
  홈 카드가 나온다 (`ACTIVITIES` 의 `hidden: true` + `buildHome()` 의 필터).
  그리기 앱에 영상이 늘 보이면 아이가 그것만 찾는다 — 부모가 설정에서 열어 준다.
  - 영상은 이름 사진과 같은 이유로 **집 NAS** 에 둔다 (저장소가 public 이고
    영상은 용량이 크다). 폴더 하나가 시리즈다:
    ```
    <NAS>/video/list.php         tools/nas-video-list.php 를 이 이름으로
    <NAS>/video/한글용사/1.mp4 … 12.mp4
    <NAS>/video/한글용사/cover.jpg    시리즈 표지 — 고르는 동안 큰 화면에 (.png 도)
    <NAS>/video/한글용사/1.jpg        그 편의 썸네일 (.png 도 된다. 없어도 된다)
    ```
    **`list.php` 를 두면 폴더만 만들면 끝이다** — 폴더를 훑어 이름과 편수를
    알려 준다. PHP 를 못 쓰면 `list.js` 에 이름만 적어도 된다
    (`window.HAICHU_VIDEOS = ['한글용사']`). 앱은 php → js 순으로 찾는다.
    - ★ **브라우저는 남의 서버 폴더 목록을 물어볼 수 없다**(보안). 파일 이름을
      알 때 가져오는 것만 된다 — 편수를 셀 수 있는 것도 `1.mp4` 라는 규칙이
      있어서다. 그래서 **폴더 이름만은** NAS 가 알려 줘야 한다.
    - 편수는 `1.mp4` 부터 있는 만큼 센다. `<video preload="metadata">` 로
      물어보는 방식이라 CORS 헤더가 필요 없다(fetch/HEAD 는 필요해서 못 쓴다).
      여덟 편씩 한꺼번에 물어보고 끊기는 자리에서 멈추며, 센 값은
      `sessionStorage` 에 남겨 그 세션 동안 다시 안 센다.
      미리 알면 `{ name, count }` 로 적어 이 과정을 건너뛸 수 있다.
    - 그림이 두 자리에 쓰인다. **칩 썸네일**은 그 편의 것 — `N.jpg` → `N.png` →
      없으면 **그 편의 3초 지점**을 캔버스에 떠서 쓴다. **폴더의 `cover.jpg/png`**
      는 칩이 아니라 **재생 자리에** 걸린다 — 무엇을 볼지 고르는 동안 큰 화면이
      비어 있지 않게. 고르면 사라진다(없으면 "무엇을 볼까요?" 글만).
      다른 origin 영상이라 캔버스가 오염돼 `toDataURL` 은 막히지만 **화면에
      그리는 건 된다** — 그래서 저장은 못 하고 열 때마다 새로 뜬다.
    - **상단이 편 목록이다. 썸네일을 골라야 재생된다** — 들어가자마자 틀지
      않는다(아이가 고르는 것부터가 이 화면의 일이다). 시리즈가 둘 이상이면
      하단 ◀ ▶ 로 시리즈를 넘긴다. 한 편이 끝나면 다음 편으로 가고 마지막
      편이 끝나면 목록으로 돌아온다. 마지막에 본 자리는 기억한다
      (`localStorage['haichu.videoAt']`).
    - **편수는 열 때마다 다시 센다.** 세어 둔 값을 기억하면 영상을 더 넣었는데
      "1편만 나온다" 가 된다 (실제로 겪었다).
  - **목록을 `fetch` 가 아니라 `<script>` 로 읽는다.** fetch 는 CORS 헤더가 있어야
    하지만 script 는 필요 없다 — 사진을 `<img>` 로 가져오는 것과 같은 요령이라
    NAS 설정을 건드릴 일이 없다. `list.js` 는 `window.HAICHU_VIDEOS = [...]` 한 줄.
  - NAS 가 꺼져 있거나 집 밖이면 조용히 "영상이 없어요" 로 남는다.
- 디자인 결정의 배경·남은 일은 옵시디언 `MJ/디자인/하이츄/` 에 있다
  (`01 Decisions` ~ `05 Next`). 디자인이 바뀌면 **저장소 사본부터** 갱신해야 코드에 반영된다.
...
7코스 130단계 완주 · 길 밖 판정 · 질러가기 방지 · 획순을 확인한다.
질러가기는 **굽은 획을 하나씩** 본다 (앞 획은 정직하게 간 뒤 그 획만 직선으로).
예전엔 한 획짜리 단계만 봐서 글자 대부분이 검사에서 빠져 있었다.
...
- **색칠의 획 좌표는 캔버스가 아니라 `밑그림 상자(artBox)` 기준**으로 저장한다
  (`pages.js` 의 `artBox(W,H)` — 1000×700 을 비율 지켜 가운데 앉힌 자리).
  x·y 를 각각 W·H 로 나눠 저장하면 화면을 돌릴 때 **종이 비율이 그대로 그림에
  먹혀서 찌그러진다** (가로 2.05 ↔ 세로 0.76). 밑그림 기준이면 칠이 그림에
  붙어 다니고, 비율이 보존되며, 종이 밖으로 나간 자리만 잘린다.
  u·v 는 **둘 다 밑그림 가로폭으로** 나눈 값이다 (같은 단위여야 비율이 산다).
  붓 굵기(`SIZES`)·이어 그리기 거리도 밑그림 가로폭 기준이다.
  - 구워 둔 baseline 과 초안(PNG)도 **상자끼리 맞춰** 옮긴다. 초안은 따로
    기록해 둘 게 없다 — `artBox` 는 캔버스 크기의 함수라 이미지 크기만으로
    그때의 상자가 되계산된다.
...
- **종이(.paper)는 상하 트레이 사이를 꽉 채우고, 배율은 "내용 상자"로 잡는다**
  (`core/fit.js` 의 `fitPaper`). 예전엔 종이를 1000×700 비율로 고정했는데,
  아이패드 미니(1133×744)에서는 가로의 3분의 1이 빈자리라 글자가 작았다.
  화면마다 실제로 쓰는 상자가 다르다 — 따라 그리기류는 코스별로 계산하고
  (미로는 판 전체, 이름은 넓고 낮은 띠), 퍼즐·다른 그림 찾기·세어보기는
  판과 카드 자리를 상수로 적어 뒀다. 좌표계는 그대로 **1000×700** 이라
  콘텐츠 파일은 손댈 일이 없다.
  - 컨텍스트 원점을 `setOrigin(ctx, fit)` 으로 옮겨 두므로 그리는 코드(`x * S`)는
    그대로다. 대신 **전체 지우기는 `clear(ctx)`**(= `clearRect(-OX,-OY,W,H)`)를
    쓸 것 — `clearRect(0,0,W,H)` 는 옮긴 만큼 가장자리를 안 지운다.
  - **캔버스의 `width` 를 넣으면 변환이 초기화된다.** 크기를 바꾼 직후에는
    반드시 `setOrigin` 을 다시 부를 것.
  - 펜은 캔버스 픽셀로 들어오므로 **들어오는 자리에서 한 번** `-OX/-OY` 해 준다.
  - 새 상자를 정할 때는 내용이 삐져나오는 만큼(길 굵기·아이콘·혹)을 여백으로
    넣을 것. 안 넣으면 가장자리가 잘린다.
...
- `hard` 는 조각 수만 정한다: 1→3 · 2→5 · 3→8 (`cut.js` 의 `PIECES`).
  **자르는 모양은 열 때마다 무작위다** — 가장 큰 조각을 반복해 가르는
  ...
  변에는 혹을 생략한다.
- **조각 가로는 `MAXW`(300) 를 넘지 못한다.** 넘으면 세로로 가른다 —
  가로로만 잘라 판만큼 넓은 띠가 나오면 오른쪽 흩는 자리(SCAT)에 못 들어가서
  판을 덮거나 화면 밖으로 반쯤 나간다. 흩을 때도 화면 안쪽으로 당기고
  (`VIEW - 조각 반폭 - 6`), 세로로는 고르게 나눠 놓는다 — 큰 조각이
  한자리에 포개지면 아래 조각이 안 보인다.
...
```js
L('c16', '바나나', 'banana', 2, 149)                 // 중: 6~10개
L('c17', '사탕만', 'candy', 3, 151, ['donut', 'cupcake'])   // 상: 방해꾼 섞임
```

- **그림 이름은 `core/icons.js` 의 `picIcon` 키다.** 새 물건을 넣으려면
  거기에 "말랑 채움" SVG 를 한 줄 더한다 (이모지는 쓰지 않는다 — 칩과
  스테이지가 같은 그림이어야 하고, 기기마다 달라지면 안 된다).
...
**음성 팩** — `assets/voice/` 에 문구별 클립(m4a, 총 153개)이 있고
...
- **더 자연스러운 목소리로 교체**: TTS 사이트(Airy Studio 등)에서 생성한
  ...
  **지금은 153문구 전부 이 방식으로 교체돼 있다** — 유나 합성은 폴백일 뿐이다.
- **교체 파일은 언제나 다시 변환한다.** 예전엔 "wav 가 m4a 보다 최신일
  때만" 바꿨는데, 문구를 고쳐 유나로 새 클립을 만든 **직후에** 좋은 목소리
  wav 를 넣으면 m4a 가 더 최신이라 영영 안 바뀐다 ("넣었는데 왜 그대로지").
  **단 git 은 깨끗하지 않다** — afconvert 가 m4a 헤더에 생성 시각을 찍어서,
  소리는 그대로인데 파일 144개가 전부 "수정됨" 으로 뜬다. 문구를 몇 개만
  추가했다면 새 클립과 manifest.json 만 남기고 나머지는 되돌릴 것:
  `cp assets/voice/manifest.json /tmp/m.json && git checkout -- assets/voice && cp /tmp/m.json assets/voice/manifest.json`
  (새로 만든 클립은 untracked 라 checkout 이 건드리지 않는다.)
...
**점 잇기 그림 추가하기** — `js/trace/dots.js` 의 `DOTS` 에 한 줄.
`pic(id, 이름, hard, 점배열)` 이고 **id 가 곧 그림 이름**이다 —
칩 아이콘을 `core/icons.js` 의 `picIcon(id)` 가 그걸로 찾으므로,
없는 그림이면 거기 "말랑 채움" SVG 를 한 줄 더해야 한다.
- **이웃한 점은 tol 의 두 배보다 멀어야 한다.** 가까우면 옆 점으로
  건너뛰어 안 이은 구간이 통과된다. `pic()` 이 가장 가까운 이웃 간격의
  절반으로 tol 을 자동 계산한다(40~66 사이로 자름 — 더 조이면 아이가
  활처럼 그은 선이 길 밖으로 나간다).
- 점이 많을수록 어렵다: 5~7 쉬움 · 8~10 보통 · 11~14 어려움.

**미로는 코스에 들어갈 때마다 새로 판다.** 같은 미로가 계속 나오면 금방
지루해진다. 칸 수(난이도 순서)·id·주인공은 그대로라 아이가 모은 별은 남는다.
- 러너의 코스 옵션 `fresh` 가 그 장치다 — `enter()` 가 `course.fresh()` 로
  단계를 다시 만들고 다시 굽는다(상자 캐시도 지운다). 미로만 쓴다.
- **가끔 해답이 곧게 이어진 시시한 판이 나온다** (560판에 한 판꼴).
  해답이 시작→도착 직선을 자주 넘나들면 자로 긋듯 쭉 그어도 완주가 된다.
  굽은 정도만 재서는 안 잡히고 **판정 엔진에 직접 그어 봐야** 안다 —
  `cheatable()` 이 그걸 하고, 걸리면 다시 판다.
- 가장 작은 3×2 는 방이 여섯뿐이라 가능한 모양이 몇 안 된다. 가끔 같은
  미로가 다시 나오는 건 정상이다 (중간 이상 크기는 매번 다르다).
...
- 글자의 마지막 획 번호(`sylSay`)에서 그 글자를 읽고(펜 떼는 순간),
  완성하면 📣 전체 듣기가 켜진다.
- **사진 칩을 누르면 그 이름을 읽어 준다.** 원래는 "완성이 보상" 이라 미리
  안 읽었는데, 얼굴 사진이 붙으면서 얼굴을 보고 고르는 화면이 됐다 —
  고를 때 "문송자" 하고 알려 주는 편이 얼굴과 이름을 잇는 데 낫다.
  완성 읽기와 📣 전체 듣기는 그대로라 보상도 남아 있다.
  읽었으면 `introSaid` 에 표시해 안내말을 건너뛴다 — 뒤이어 나오면
  **방금 읽은 이름을 잘라 먹는다** (클립은 새로 틀 때 앞엣것을 끊는다).
- **이름 사진**: 위쪽 칩이 하트 대신 그 사람 얼굴이 된다. 사진은 저장소가 아니라
  **집 NAS** 에 둔다 (`trace/index.js` 의 `FACE_BASE`) — 이 저장소는 public 이라
  가족 사진을 커밋할 수 없다. 주소에 이름을 그대로 붙이는 규칙이라, 사진을
  추가할 때 코드는 건드리지 않는다. 코스 옵션은 `photos:true`.
  없는 이름·NAS 꺼짐·오프라인이면 조용히 하트로 남는다 (`<img>` 의 onerror).
  `<img>` 로 보여 주기만 하므로 CORS 설정이 필요 없고, https 라 GitHub Pages
  에서도 혼합 콘텐츠로 막히지 않는다. 서비스 워커는 다른 origin 에 안 끼어든다.
- 코스 옵션 `full`(전체 듣기 흐름) · `road/fill/icon/badge`(가는 그리기 치수)는
  이름 쓰기가 쓴다. 블록이 작아 기본 굵기(62)면 이웃 획을 덮는다.

**영어는 한 단계가 대문자+소문자 한 쌍이다.** 왼쪽에 대문자(LX=300), 오른쪽에
소문자(RX=800)를 놓고 획순은 대문자부터다. 칩도 위에 대문자, 아래 소문자를
같이 보여 준다 — 레벨에 `lbl` 을 주면 러너가 `.lvl.pair` 칩으로 그린다
(좁은 화면에서 라벨을 숨기는 규칙에서 빠진다. 소문자는 이름표가 아니라 내용이다).
- 소문자는 네 줄 공책 기준선으로 그린다: `TOP 145`(윗기둥) · `XT 347`(몸통 위) ·
  `BASE 565`(밑줄) · `DESC 695`(꼬리). 대문자와 모양이 같은 **c·o·s·u·v·w·x·z 는
  `smallOf()` 로 대문자를 밑줄 기준으로 줄여 쓴다** — 따로 그릴 이유가 없다.
- 글자 크기는 그대로 두고 자리만 옮긴다. 화면 배율은 러너가 코스 전체 상자로
  잡으므로(`core/fit.js`) 두 글자가 들어가도 알아서 맞춰진다.
...
**획순의 근거는 [docs/획순.md](docs/획순.md) 에 있다.** 한글·영어·숫자 모두
공식 표준이 없어서(한글 맞춤법은 자모의 수·순서·이름만 정한다) 통용 관행을 따랐고,
규칙 원본은 `docs/stroke_order.json` 이다 — 글자마다 획수·방향·다른 교본의 방식이 적혀 있다.
...
- **"네트워크 우선" 이라고 최신이 보장되지 않는다.** 서비스 워커 안의 `fetch()` 도
  브라우저 HTTP 캐시를 먼저 본다. GitHub Pages 는 `cache-control: max-age=600` 을
  주므로 그 사이에는 서버에 가지도 않는다. `sw.js` 의 `netFirst()` 가
  `cache:'no-cache'` 로 항상 재검증하는 이유 (안 바뀐 파일은 304 라 거의 공짜).
- **소리가 "그 사이트에서만" 안 난다면 사이트 데이터를 의심할 것.**
  로컬 서버(`192.168.x.x`)에선 나는데 GitHub Pages 에선 무음이었던 적이 있다.
  코드·음원은 양쪽이 같았고, 홈 진단 줄이 `컨텍스트:suspended · 킥:멈춤` 이었다
  — 즉 `<audio>` 재생과 컨텍스트 깨우기가 **둘 다 거부**된 상태.
  **사이트 데이터(쿠키·캐시)를 지우니 해결됐다.** 사파리는 origin 단위로
  자동재생 허용·음소거·저장소를 따로 들고 있어서, 한쪽만 오염될 수 있다.
  조치 순서: ① 주소창 「ㅏA」 → 웹사이트 설정 → 자동 재생 허용
  ② 설정 → Safari → 방문 기록·데이터 지우기 ③ 홈 화면 아이콘 재추가.
...
- **`roundRect` 는 iPadOS 16.4 부터다.** 그 아래 버전에서는 그리다 예외가 나서
  ...
  iOS 지원 버전을 확인할 것.
- **캔버스에 찍을 SVG 에는 `xmlns` 가 있어야 한다.** HTML 안에 그대로 넣을 때(칩)는
  없어도 되지만, data URI 로 `<img>` 에 물리면 없는 순간 **조용히 로드 실패**한다
  (onerror 를 안 보고 있으면 "그림만 안 나오네" 로 끝난다 — 실제로 겪었다).
  `trace/index.js` 의 `bake()` 가 구울 때 끼워 넣는다.
