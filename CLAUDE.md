...
**iOS TTS 는 함정이 세 개다** — `say()` 가 전부 우회하니 직접 speak 하지 말 것.
① 사용자 터치 안에서 한 번 시작해 줘야 이후 자동 호출이 허용된다
   (첫 터치의 `unlock()` 이 무음 한 마디로 잠금을 푼다).
② `cancel()` 직후의 `speak()` 는 삼켜진다 — 한 박자(60ms) 띄운다.
