...
- **페이지의 첫 speak 은 사용자 제스처의 "동기" 흐름 안에 있어야 한다.**
  await 한 번만 지나도 제스처 밖이라 막히고, 막히면 그 뒤의 speak 도 계속 무음이다.
  제스처 안에서 한 번 speak 하고 나면 지연·비동기 speak 도 다 허용된다.
  `unlock()` 이 터치마다 빈 발화를 동기로 흘려 세션을 푼다 (풀리면 그만 시도).
