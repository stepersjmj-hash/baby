...
js/
  ...
  core/
    icons.js              SVG 아이콘 전부 — 홈 카드 · 트레이 · 색칠 도구 (이모지 대체)
...
홈 화면 디자인의 **기준 원본은 `design_handoff_home_redesign/README.md`** 다.
색·타이포·간격·아이콘 path 가 최종값으로 적혀 있으니, 홈을 고칠 일이 생기면
...
- **아이콘은 전부 `js/core/icons.js` 한 곳에 있다.** 세 갈래다:
...
- 따라 그리기 스테이지의 출발·도착 그림은 아직 캔버스 이모지다 (`trace/index.js` 의 `emoji()`).
