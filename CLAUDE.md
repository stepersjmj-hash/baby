...
- **speak 은 반드시 사용자 제스처(터치 이벤트 핸들러)의 "동기" 흐름 안에서 부른다.**
  setTimeout·await 한 번만 지나도 제스처 밖이라 무음이 된다. 그래서 완성 읽기는
  판정 시점이 아니라 **펜을 떼는 pointerup 안**에서 한다 (`sayOnLift` 표시 →
  `onEnd` 에서 동기 호출). 빈 발화로 세션을 미리 푸는 꼼수는 쓰지 않는다 —
  기기에 따라 안 풀리거나 큐를 막는다.
