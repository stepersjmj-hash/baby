...
- **아이콘은 전부 `js/core/icons.js` 한 곳에 있다.** 네 갈래다:
  ...
  선으로, 미로 14종은 목적지를 채움으로). 없는 키면 빈 문자열이라 러너가
  `level.ico` 로 되돌아간다. 여기에 스테이지 위 그림 `STAGE_ART` 가 더해진다.
...
- 단계 칩의 난이도 띠는 레벨의 `hard`(1·2·3)로 켜진다. 따라 그리기류에서는
  **선 긋기만** 난이도가 있다 (1~8 쉬움 · 9~15 보통 · 16~22 어려움).
- 단계 칩은 `.lvl` 하나를 네 화면이 공유한다. 퍼즐 칩은 그 문제의 그림을 캔버스로
  줄여 넣는다(`drawScene`). 세어보기·다른 그림 찾기 칩의 이모지는 **UI 가 아니라
  콘텐츠**(그 문제의 물건·장면)라 그대로 둔다.
...
- 스테이지 위 그림은 코스 옵션 `art`(`{from,to}`)로 정한다 — `pen`·`star`·`runner` 는
  ...
  **선 긋기·점 잇기의 캐릭터 쌍(🐝→🌻)만 이모지로 남겼다** — UI 가 아니라
  그 단계의 이야기라서다(세어보기 칩 이모지와 같은 이유).
